...
try:
    print(sys.argv[1])
    sys.path.insert(0, sys.argv[2] + '/functions/')
    from tune import tune 

    with open(sys.argv[2] + '/configuration/devries2023_model_config.yml', 'r') as f:  #CHANGE TO YOUR CONFIG FILE
    ...
    from tune import tune 
    with open('/home/phyto/planktonSDM/configuration/devries2023_model_config.yml', 'r') as f: #CHANGE TO YOUR CONFIG FILE
        model_config = load(f, Loader=Loader)
...
#run model:
m.train(model=sys.argv[3], classifier=True)
